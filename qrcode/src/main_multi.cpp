...
string cell_result;
vector<string> cell_results;  //用来记录每次的异常信息
int res = -1;
...
int test_image_multi(Image& image, bool hybrid)
{
  ...
  try
  {
    ...
    vector<Ref<Result> > results(decodeMultiple(binary, hints));
    for(size_t i = 0; i < results.size(); i++)
    {
    	cell_results.push_back(results[i]->getText()->getText());
    }
    cell_result = "detect qrcode";   
    res = 0;
  }
  ...
  if(res==0)
  {
	  ...
	  {
		  v.push_back(cell_results);
		  for(size_t j = 0; j < cell_results.size(); j++)
		  {
		 	 counter++;
		 	 ...
		 	 out <<counter<<","<<time<<","<<cell_results[j]<<","<<x<<","<<y<<","<<"0.5"<<endl;
		  }
		  srv.request.object.header.frame_id = "base_link";
	          ...
	          srv.request.object.info.name = cell_results[0];
